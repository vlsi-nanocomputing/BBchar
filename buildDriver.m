...
switch(driverSettings.sweepType)
    ...
    case 'log'
        half_pos_variation = logspace(-2, 0, driverSettings.NsweepSteps/2); % generates NsweepSteps/2 points between decades 10^-2 and 10^0.
        overturned_half_pos_variation = logspace(0, -2, driverSettings.NsweepSteps/2);
...
% The length of a fixed value driver must complain with:
...
for ii = 1:Ndrivers
     ...
     switch driverSettings.driverModes{ii}
         case '0'
             [valuesDr{ii,2:end}] = fixed_zero{:};
         case '1'
             [valuesDr{ii,2:end}] = fixed_one{:};
         case 'sweep'
             [valuesDr{ii,2:end}] = dr_pos_sweep{:};
         case 'not_sweep'
             [valuesDr{ii,2:end}] = dr_neg_sweep{:};
     end
     ...
     if driverSettings.doubleMolDriver 
         switch driverSettings.driverModes{ii} 
             ...
             case '0'
                 [valuesDr{ii+Ndrivers,2:end}] = fixed_one{:};
             case '1'
                 [valuesDr{ii+Ndrivers,2:end}] = fixed_zero{:};
             case 'sweep'
                 [valuesDr{ii+Ndrivers,2:end}] = dr_neg_sweep{:};
             case 'not_sweep'
                 [valuesDr{ii+Ndrivers,2:end}] = dr_pos_sweep{:};
         end
     end
