...
function characterization(charSettings,file,terminationSettings,terminationCircuit,driverPara,BBcharPath)
    ...
    A = golden_outMol_finder(simulation_path, bus_flag, term_angle);

    % saving the names of the drivers in a string array
    N_drivers = tableNMol.stack_driver.num;
    for dd = 1:N_drivers
        tmp_driver_label = tableNMol.stack_driver.stack(dd).identifier_qll;
        driver_labels(dd,:) = strcat('driver_',tmp_driver_label);
    end
    ...
    for a = 2:length(table.Properties.VariableDescriptions)        % scanning the names of the original table's columns
        ...
        for c = 1:N_drivers
            if strcmp(heading,driver_labels(c,:)) == 1
    ...
    saved_drivers = NaN(length(driver_mat),N_drivers);    %preallocating driver array

    for j = 1:N_drivers
        dr_table{j} = driver_labels(j,:);     %table containing the names of the drivers
    end
    dr_array = string(dr_table);    %array containing the names of the drivers (it's fixed)

    
    %%%%%%%%    generating a .csv file for each output    %%%%%%%%
    for d = 1:A.N_outputs
        headers_array = [A.output_labels(d,:) dr_array];   %temporary array of strings containing the headers of the .csv file
        for e = 1:length(clock_mat)
            if clock_mat(e,d) == 2 && original_clock_mat(e,d) == 2      %check if there is HOLD state
        ...
        file_label = A.output_labels(d,:);
        file_name = strcat(file_label,'.csv');
    ...
    for w = 1:A.N_outputs
        fprintf(fileid,'%s ',A.output_labels(w,:));
    end
