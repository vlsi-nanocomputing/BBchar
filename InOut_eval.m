...
function [Vout] = InOut_eval(sim_path,file,input)
    ...
    Vout = NaN(1,N_outputs);   %preallocating the temporary Vout array (it will contain as many values as the number of outputs)
    outputs = cell2mat(split(fgetl(fileid)));   %gets the names of the outputs in a string column array

    for j = 1:N_outputs
        if strcmp(outputs(j,:),'Vout_A') || strcmp(outputs(j,:),'Vout_C')
            ...
            Vout(j) = out_mat(pos_near,1);    %final output value is the closest one with respect to the average value
        end
