...
for nOut = 1:Noutputs
    ...
    if exist('test_x','var') && any([CUTstruct.molecules.x] == test_x)
        index = find([CUTstruct.molecules(:).x] == test_x);
        nearMolPhase = CUTstruct.molecules(index(1)).phase;
    elseif exist('test_y','var') && any([CUTstruct.molecules.y] == test_y)
...
%%% Function used to generate the new qll file with the termination
